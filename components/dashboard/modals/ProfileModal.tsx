...
export default function ProfileModal({ isOpen, onClose, profile, onSuccess }: ProfileModalProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}user/update-architect`, { method: "PUT", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` }, body: JSON.stringify(payload) });
      const data = await res.json();
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-4 sm:gap-5">
                ...
                {isSearching ? <div className="p-3 text-sm text-zinc-500 text-center">Searching...</div> : suggestions.length > 0 ? (
                  <ul className="py-1">
                    {suggestions.map((item) => (
                      <li key={item.place_id} onClick={() => selectLocation(item)} className="px-4 py-2.5 text-sm text-black dark:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer border-b border-zinc-100 dark:border-zinc-800 last:border-0">{item.display_name}</li>
                    ))}
                  </ul>
          ...
          <div className="flex gap-4 mt-6 pt-6 border-t border-zinc-100 dark:border-zinc-800">
            ...
            <button type="submit" disabled={isSaving} className="flex-1 py-3 bg-black text-white rounded-xl font-bold flex justify-center items-center">
              {isSaving ? <LoadingSpinner className="w-5 h-5" /> : "Update Profile"}
            </button>
          </div>
        </form>
      </div>
